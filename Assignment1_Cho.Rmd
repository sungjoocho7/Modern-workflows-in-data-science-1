...
# date variable (month-day-year format)
...
```{r}
# create log number of cases
covid_long$logcase <- log(covid_long$case)
covid_long$logcase <- ifelse(covid_long$logcase == -Inf, 0, covid_long$logcase)

plot_overall_change <- ggplot(covid_long, aes(x = date, y = logcase)) +
  geom_line() +
  geom_point() +
...
# plot
plot_overall_change <- ggplot(covid_long, aes(x = date, y = log(case))) +
...
```{r}
# table
tab_change_ctry_rate_infection <- covid_long %>%
  group_by(Country_Region, date) %>%
...
# plot
plot_change_ctry_rate_infection <- ggplot(tab_change_ctry_rate_infection, 
                                aes(x = date, 
                                    y = (sum_case/population)*100000, 
                                    color = Country_Region)) +
  geom_line() +
  ...
  theme(legend.position = "none")
plot_change_ctry_rate_infection
